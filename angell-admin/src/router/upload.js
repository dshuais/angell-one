...
const { uploadImg, uploadFile, downloadFile, } = require('../controller/upload.controller')
const { auth, hadAdminPermission } = require('../middleware/auth.middleware')
const { startUpload, dowValid, } = require('../middleware/upload.middleware')
const router = new Router(), dayjs = require('dayjs'), dirName = dayjs().format('YYYYMMDD')


// 商品相关模块路由
// 上传商品图片  auth,
router.post('/picture', auth, KoaBody({ // 把开启上传中间件放在路由上 解决挂载全局全部接口都可上传的问题
  ...
  formidable: {
    ...
    onFileBegin: (name, file) => {
      ...
      const dir = path.join(__dirname, `../../../uploads/img/${dirName}`)
...
router.post('/file', KoaBody({
  ...
  formidable: {
    ...
    //   // uploadDir不能使用相对路径 因为他相对于process.cwd() 并不是相对于当前文件夹
    uploadDir: path.join(__dirname, '../../../uploads/file/'),
    keepExtensions: true, // 保留扩展名
    onFileBegin: (name, file) => {
      // console.log('进入了重命名', name)
      // console.log('进入了重命名2', file)
      // const dirName = dayjs().format('YYYYMMDD') // 文件夹内按照日期存放图片
      const dir = path.join(__dirname, `../../../uploads/file/${dirName}`)
      if (!fs.existsSync(dir)) fs.mkdirSync(dir)
      const filename = file.originalFilename.replaceAll(' ', '_').replace(/[`~!@#$%^&*()|\-=?;:'",<>\{\}\\\/]/gi, '_')
      file.name = filename
      file.filepath = `${dir}/${filename}`
